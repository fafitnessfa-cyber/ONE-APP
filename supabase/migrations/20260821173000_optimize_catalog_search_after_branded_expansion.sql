...
create index if not exists catalog_foods_active_brand_name_normalized_pattern_idx
on public.catalog_foods using btree (public.normalize_food_search_text(brand_name) text_pattern_ops)
where is_active and brand_name is not null;

create index if not exists catalog_foods_active_name_normalized_idx
on public.catalog_foods using btree (public.normalize_food_search_text(name))
where is_active;

create index if not exists catalog_foods_active_name_idx
on public.catalog_foods using btree (name)
where is_active;

create index if not exists catalog_foods_active_brand_name_normalized_idx
on public.catalog_foods using btree (public.normalize_food_search_text(brand_name))
where is_active and brand_name is not null;

create index if not exists food_aliases_alias_normalized_pattern_idx
on public.food_aliases using btree (public.normalize_food_search_text(alias) text_pattern_ops);

create index if not exists food_aliases_alias_normalized_idx
on public.food_aliases using btree (public.normalize_food_search_text(alias));
...
begin
  ...
  if v_fast_candidate_count >= v_capped_limit then
    ...
    alias_rank as (
      ...
      from public.food_aliases fa
      join fast_candidate_scores fcs on fcs.id = fa.food_id
      group by fa.food_id
    ),
    scored as (
      select
      ...
      from fast_candidate_scores fcs
      join public.catalog_foods cf on cf.id = fcs.id
  ...
  name_trigram_candidates as (
    ...
    from (
      select
        ...
        (
          greatest(
            similarity(lower(cf.name), v_raw_query_for_trgm),
            case
              when v_query_length >= 5
                then word_similarity(v_raw_query_for_trgm, lower(cf.name))
              else 0
            end
          ) * 7000
        )::double precision as branch_score,
  ...
  brand_trigram_candidates as (
    ...
    from (
      select
        ...
        (
          greatest(
            similarity(lower(cf.brand_name), v_raw_query_for_trgm),
            case
              when v_query_length >= 5
                then word_similarity(v_raw_query_for_trgm, lower(cf.brand_name))
              else 0
            end
          ) * 6500
        )::double precision as branch_score,
  ...
  fuzzy_fallback_candidates as (
    ...
    from (
      select
        ...
        (
          greatest(
            word_similarity(v_raw_query_for_trgm, lower(cf.name)),
            case
              when cf.brand_name is not null
                then word_similarity(v_raw_query_for_trgm, lower(cf.brand_name))
              else 0
            end
          ) * 6000
      ...
      where v_query_length >= 5
        ...
        and not exists (select 1 from candidate_scores)
        and greatest(
          word_similarity(v_raw_query_for_trgm, lower(cf.name)),
          case
            when cf.brand_name is not null
  ...
  all_candidates as materialized (
    select id, branch_score from candidate_scores
    union all
    select id, branch_score from fuzzy_fallback_candidates
  ),
  alias_rank as (
    select
      fa.food_id,
    ...
    from public.food_aliases fa
    join all_candidates ac on ac.id = fa.food_id
    group by fa.food_id
  ),
  scored as (
    select
    ...
    join public.catalog_foods cf on cf.id = ac.id
    join public.food_sources fs on fs.id = cf.source_id
    cross join lateral (
      select public.normalize_food_search_text(cf.name) as normalized_name
    ) name_data
