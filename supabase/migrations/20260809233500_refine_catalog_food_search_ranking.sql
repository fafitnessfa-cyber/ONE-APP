...
as $$
  ...
  trigram_candidates as (
    ...
    where qs.query_length >= 3
      ...
      and (
        lower(cf.name) % qs.raw_query_for_trgm
        or (
          qs.query_length >= 5
    ...
    where qs.normalized_query <> ''
      ...
      and (
        ...
        or (
          qs.query_length >= 3
          and (
            lower(fa.alias) % qs.raw_query_for_trgm
            or (
              qs.query_length >= 5
              and word_similarity(qs.raw_query_for_trgm, lower(fa.alias)) >= 0.42
            )
          )
        )
      )
  ...
  scored as (
    select
      ...
      cardinality(regexp_split_to_array(public.normalize_food_search_text(cf.name), ' ')) as normalized_name_token_count,
      qs.query_token_count,
      qs.query_tokens,
      qs.capped_limit,
      (public.normalize_food_search_text(cf.name) = qs.normalized_query) as exact_name_match,
      (public.normalize_food_search_text(cf.name) like qs.normalized_query || '%') as name_prefix_match,
      ...
      case
        when qs.query_length >= 3
          then position(qs.normalized_query in public.normalize_food_search_text(cf.name)) > 0
        else false
      end as contains_match,
      case
        when qs.query_token_count = 0 then false
        else qs.query_tokens <@
          (regexp_split_to_array(public.normalize_food_search_text(cf.name), ' '))[
            1:least(
              cardinality(regexp_split_to_array(public.normalize_food_search_text(cf.name), ' ')),
              qs.query_token_count + 1
            )
          ]
  ...
  select
    ...
    (
      ...
      case when leading_token_window_match and taxonomy_style_name then 18000 else 0 end +
      case
        when name_prefix_match and query_token_count >= 2 then 8000
        when name_prefix_match then 3000
        else 0
      end +
      case
        when alias_prefix_match and query_token_count >= 2 then 7000
        when alias_prefix_match then 2500
        else 0
      end +
      case when phrase_match then 5000 else 0 end +
      (fulltext_rank * 10000) +
      (greatest(fuzzy_similarity, alias_fuzzy_similarity) * 1000) +
      case when contains_match then 100 else 0 end -
      (greatest(normalized_name_token_count - query_token_count, 0) * 10) -
  ...
  order by
    ...
    leading_token_window_match desc,
    taxonomy_style_name desc,
    name_prefix_match desc,
    alias_prefix_match desc,
    phrase_match desc,
    fulltext_rank desc,
    greatest(fuzzy_similarity, alias_fuzzy_similarity) desc,
    contains_match desc,
    greatest(normalized_name_token_count - query_token_count, 0) asc,
