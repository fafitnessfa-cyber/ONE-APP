...
as $$
  ...
  select
    ...
    (
      ...
      case
        when taxonomy_style_name and coalesce(input_food_count, 9999) <= 1 then 9000
        when taxonomy_style_name and coalesce(input_food_count, 9999) = 2 then 5000
        else 0
      end +
      case
  ...
  order by
    ...
    coalesce(input_food_count, 9999) asc,
    name_prefix_match desc,
    alias_prefix_match desc,
    phrase_match desc,
    leading_fuzzy_token_similarity desc,
