drop function if exists public.search_catalog_foods(text, integer, text, text, text);
drop function if exists public.search_catalog_foods(text, integer, text, text, text, text);
...
as $$
  with input as (
    select
      ...
      nullif(
        ...
        ''
      ) as normalized_preferred_market_country_code
  ),
  query_state as (
    select
      normalized_query,
      ...
      normalized_preferred_market_country_code,
  ...
  brand_prefix_candidates as (
    ...
    where qs.normalized_query <> ''
      ...
      and public.normalize_food_search_text(cf.brand_name) like qs.normalized_query || '%'
  ),
  fts_candidates as (
    select cf.id
    from public.catalog_foods cf
  ...
  brand_trigram_candidates as (
    ...
    where qs.query_length >= 3
      ...
      and lower(cf.brand_name) % qs.raw_query_for_trgm
  ),
  alias_candidates as (
  ...
  indexed_candidate_ids as (
    ...
    union
    select id from brand_prefix_candidates
    union
    select id from fts_candidates
    union
    select id from trigram_candidates
    union
    select id from brand_trigram_candidates
    union
  ...
  scored as (
    select
      ...
      (
        brand_data.normalized_brand <> ''
        and brand_data.normalized_brand = qs.normalized_query
      ) as exact_brand_match,
      (
        brand_data.normalized_brand <> ''
        and brand_data.normalized_brand like qs.normalized_query || '%'
      ) as brand_prefix_match,
      ...
      greatest(
        case
          when qs.query_length >= 3 and brand_data.normalized_brand <> '' and lower(cf.brand_name) % qs.raw_query_for_trgm
            then similarity(lower(cf.brand_name), qs.raw_query_for_trgm)
          else 0
        end,
        case
          when qs.query_length >= 5 and brand_data.normalized_brand <> ''
            then word_similarity(qs.raw_query_for_trgm, lower(cf.brand_name))
          else 0
        end
      ) as brand_fuzzy_similarity,
      case
        when qs.query_length >= 3
      ...
      case
        when qs.query_length >= 3 and brand_data.normalized_brand <> ''
          then position(qs.normalized_query in brand_data.normalized_brand) > 0
        else false
      end as brand_contains_match,
      case
      ...
      (
        qs.normalized_preferred_market_country_code is not null
        and upper(coalesce(cf.country_code, '')) = qs.normalized_preferred_market_country_code
      ) as preferred_market_match,
    ...
    cross join lateral (
      select public.normalize_food_search_text(coalesce(cf.brand_name, '')) as normalized_brand
    ) brand_data
    cross join lateral (
      select
  ...
  ranked as (
    select
      ...
      (
        ...
        case when exact_brand_match then 18000 else 0 end +
        case
        ...
        case
          when brand_prefix_match and query_token_count >= 2 then 12000
          when brand_prefix_match then 5000
          else 0
        end +
        case when brand_contains_match then 1200 else 0 end +
        case
          when food_type = 'branded' and query_token_count >= 2 and (exact_brand_match or brand_prefix_match)
            then 4000
          else 0
        end +
        ...
        (brand_fuzzy_similarity * 2500) +
        case
          when preferred_market_match and food_type = 'branded' then 350
          when preferred_market_match then 125
          else 0
        end +
        (coalesce(data_completeness, 0) * 500) +
        case
          when query_token_count = 1
            and food_type in ('common', 'ingredient')
            and not exact_brand_match
            and not brand_prefix_match
            and brand_fuzzy_similarity < 0.55
            then 2600
          else 0
        end +
        case
          when query_token_count = 1
            and food_type = 'branded'
            and not exact_name_match
            and not name_prefix_match
            and not exact_brand_match
            and not brand_prefix_match
            and brand_fuzzy_similarity < 0.55
            and not category_prefix_match
            then -5000
          else 0
        end +
        case
