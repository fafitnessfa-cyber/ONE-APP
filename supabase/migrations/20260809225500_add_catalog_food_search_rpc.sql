create or replace function public.normalize_food_search_text(input text)
returns text
language sql
immutable
strict
...
create index catalog_foods_name_normalized_pattern_idx
on public.catalog_foods (
  public.normalize_food_search_text(name) text_pattern_ops
);
...
as $$
  ...
  scored as (
    select
      ...
      coalesce(ar.alias_trigram_similarity, 0) as alias_trigram_similarity,
  ...
  select
    ...
    (
      ...
      case when name_prefix_match then 50000 else 0 end +
      case when exact_alias_match then 40000 else 0 end +
      case when alias_prefix_match then 30000 else 0 end +
      case when phrase_match then 20000 else 0 end +
      (fulltext_rank * 10000) +
      (greatest(trigram_similarity, alias_trigram_similarity) * 1000) +
  ...
  order by
    ...
    phrase_match desc,
    fulltext_rank desc,
    greatest(trigram_similarity, alias_trigram_similarity) desc,
    contains_match desc,
