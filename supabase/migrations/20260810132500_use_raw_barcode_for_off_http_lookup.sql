...
declare
  ...
  external_lookup_barcode text := regexp_replace(trimmed_raw_barcode, '[^0-9]', '', 'g');
...
begin
  ...
  if external_lookup_barcode = '' or not public.is_valid_gtin(external_lookup_barcode) then
    external_lookup_barcode := trimmed_normalized_barcode;
  end if;
